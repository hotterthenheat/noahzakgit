...
// ----------------------------------------------------------------
function PressureGauge({ gauge }: { gauge: any }) {
  const p = gauge?.pressure ?? 0;
  const pct = (p + 100) / 2; // 0..100
  const color = p >= 25 ? '#30d158' : p <= -25 ? '#ff453a' : '#ff9f0a';

  return (
    <div className="space-y-3" id="dealer-pressure-gauge-container">
      <div className="relative h-3.5 rounded-full bg-gradient-to-r from-rose-500/25 via-zinc-800 to-emerald-500/25 border border-zinc-800 overflow-visible">
      ...
      <div className="flex justify-between text-[8px] font-black tracking-widest text-zinc-600 uppercase">
        <span className="text-rose-400/80">Dealer Selling</span>
        <span style={{ color }} className="text-[13px] font-mono leading-none font-bold">
          {p > 0 ? '+' : ''}
          {p}
        </span>
        <span className="text-emerald-400/80">Dealer Buying</span>
      </div>
    </div>
  );
}
...
// ----------------------------------------------------------------
function VolatilityEngineCard({ vol, expectedMovePct, spot }: { vol: any; expectedMovePct?: number; spot?: number }) {
  if (!vol) return null;
  const regimeColor =
    vol.regime === 'EXPANSION' ? 'text-rose-400 border-rose-500/30 bg-rose-500/10'
    : vol.regime === 'COMPRESSION' ? 'text-sky-400 border-sky-500/30 bg-sky-500/10'
    : 'text-zinc-300 border-zinc-700 bg-zinc-900';

  return (
    <div className="apple-glass rounded-lg p-4 space-y-3.5" id="volatility-engine-card">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {vol.squeeze && (
            <span className="px-1.5 py-0.5 rounded-xs text-[7.5px] font-black tracking-widest uppercase bg-sky-500/15 border border-sky-500/40 text-sky-300 animate-pulse">
              SQUEEZE ARMED
            </span>
          )}
      ...
      {/* Energy bar */}
      <div>
        <div className="flex justify-between text-[8px] text-zinc-500 font-bold uppercase tracking-widest mb-1">
          <span>Expansion Energy</span>
          <span className="text-white font-mono text-[10px] font-black">{vol.energy}/100</span>
        </div>
        <div className="h-2 rounded-full bg-zinc-900 border border-zinc-850 overflow-hidden">
          <motion.div
            className="h-full rounded-full bg-gradient-to-r from-sky-500 via-amber-400 to-rose-500"
            animate={{ width: `${vol.energy}%` }}
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="bg-black/40 border border-zinc-900 rounded-sm py-2">
          <div className="text-[7.5px] text-zinc-600 font-black uppercase tracking-widest">ATR %ile</div>
          <div className="text-[13px] font-mono text-white font-bold">{vol.atrPercentile}</div>
        </div>
        <div className="bg-black/40 border border-zinc-900 rounded-sm py-2">
          <div className="text-[7.5px] text-zinc-600 font-black uppercase tracking-widest">ATR Slope</div>
          <div className={`text-[13px] font-mono font-bold ${vol.atrSlope >= 1 ? 'text-rose-400' : 'text-sky-400'}`}>
            {vol.atrSlope}×
          </div>
        </div>
        <div className="bg-black/40 border border-zinc-900 rounded-sm py-2">
          <div className="text-[7.5px] text-zinc-600 font-black uppercase tracking-widest">Exp Move</div>
...
export function DealerFlowView() {
  ...
  return (
    <div className="w-full space-y-4 stagger-children" id="dealerflow-main-workspace-view">
      {/* ============== HEADER STRIP ============== */}
      <div className="apple-glass rounded-lg px-5 py-4 flex flex-col lg:flex-row lg:items-center gap-4 justify-between scanline-overlay" id="dealerflow-header-strip">
        <div className="flex items-center gap-3.5">
          <div className="w-9 h-9 rounded-md bg-gradient-to-br from-emerald-500/20 to-sky-500/10 border border-emerald-500/30 flex items-center justify-center">
            <Waves className="w-4.5 h-4.5 text-emerald-400" />
          </div>
          <div>
            ...
            <p className="text-[9px] text-zinc-550 uppercase tracking-widest mt-0.5">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          {[
            { label: 'Net GEX', value: profile ? fmtBn(profile.netGex) : '—', tone: profile?.netGex >= 0 ? 'text-emerald-400' : 'text-rose-400', icon: <Layers className="w-3 h-3" /> },
            { label: 'Call Wall', value: profile?.callWall?.toFixed(0) ?? '—', tone: 'text-emerald-300', icon: <ArrowUpRight className="w-3 h-3" /> },
            { label: 'Put Wall', value: profile?.putWall?.toFixed(0) ?? '—', tone: 'text-rose-300', icon: <ArrowDownRight className="w-3 h-3" /> },
            { label: 'γ-Flip', value: profile?.gammaFlip?.toFixed(0) ?? '—', tone: 'text-amber-300', icon: <Crosshair className="w-3 h-3" /> },
            { label: 'Pin Magnet', value: profile?.magnet?.toFixed(0) ?? '—', tone: 'text-sky-300', icon: <Magnet className="w-3 h-3" /> },
          ].map(card => (
            <div key={card.label} className="bg-black/50 border border-zinc-900 rounded-md px-3 py-2 min-w-[86px]" id={`card-${card.label.toLowerCase().replace(/\s+/g, '-')}`}>
              <div className="flex items-center gap-1 text-[7.5px] font-black tracking-widest text-zinc-500 uppercase">
                {card.icon}
                {card.label}
              </div>
              <div className={`text-[13px] font-mono font-bold ${card.tone}`}>{card.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4" id="dealerflow-main-grid">
        ...
        <div className="xl:col-span-2 apple-glass rounded-lg p-5" id="gex-profile-chart-panel">
          ...
          {/* OI footer */}
          {profile && (
            <div className="mt-4 pt-3 border-t border-zinc-900 grid grid-cols-3 gap-2 text-center" id="gex-profile-chart-oi-footer">
              <div>
                <div className="text-[7.5px] text-zinc-630 font-black uppercase tracking-widest">Call OI</div>
                <div className="text-[12px] font-mono text-emerald-300 font-bold">{profile.totalCallOi?.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-[7.5px] text-zinc-630 font-black uppercase tracking-widest">Put OI</div>
                <div className="text-[12px] font-mono text-rose-300 font-bold">{profile.totalPutOi?.toLocaleString()}</div>
              </div>
              <div>
                <div className="text-[7.5px] text-zinc-630 font-black uppercase tracking-widest">C/P Ratio</div>
        ...
        <div className="space-y-4" id="dealerflow-right-rail-panel">
          {/* Dealer pressure */}
          <div className="apple-glass rounded-lg p-4 space-y-3.5">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-[9px] font-black tracking-widest text-zinc-400 uppercase">
          ...
          {/* Volatility engine */}
          <VolatilityEngineCard vol={disp?.volatility} expectedMovePct={profile?.expectedMovePct} spot={profile?.spot} />

          {/* Structure read */}
          <div className="apple-glass rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-[9px] font-black tracking-widest text-zinc-400 uppercase">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {disp?.structure?.pricePosition && (
                  <span className="px-2 py-0.5 rounded-xs text-[8px] font-black tracking-widest uppercase border bg-zinc-900 border-zinc-750 text-zinc-300">
                    {disp.structure.pricePosition}
                  </span>
                )}
              </div>
            ...
            <div className="space-y-1 max-h-[120px] overflow-y-auto pr-1">
              {(disp?.structure?.events || []).slice(-6).reverse().map((e: any) => (
                <div key={e.id} className="flex items-center justify-between text-[9px] font-mono bg-black/40 border border-zinc-900 rounded-sm px-2 py-1">
                  <span className={`font-black ${e.kind === 'CHoCH' ? 'text-amber-400 font-bold' : e.direction === 'bullish' ? 'text-emerald-400' : 'text-rose-400'}`}>
              ...
              {(!disp?.structure?.events || disp.structure.events.length === 0) && (
                <div className="text-zinc-600 text-[9px] italic text-center py-2">No confirmed breaks in window</div>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" id="dealerflow-displacement-row">
        {/* Displacement zones */}
        <div className="apple-glass rounded-lg p-4" id="displacement-zones-panel">
          <div className="flex items-center gap-2 text-[9px] font-black tracking-widest text-zinc-400 uppercase mb-3">
            <Zap className="w-3.5 h-3.5 text-amber-400" />
            Displacement Zones
            <span className="text-zinc-650 ml-auto font-mono text-[8px]">{(disp?.zones || []).length} tracked</span>
          </div>
          <div className="space-y-1.5 max-h-[230px] overflow-y-auto pr-1">
            {(disp?.zones || []).slice().reverse().map((z: any) => (
              <div key={z.id} className="bg-black/40 border border-zinc-900 rounded-sm px-2.5 py-1.5">
                <div className="flex items-center justify-between">
                  <span className={`text-[9px] font-black uppercase ${z.type === 'bullish' ? 'text-emerald-400' : 'text-rose-400'}`}>
                ...
                <div className="flex items-center gap-3 mt-1 text-[8px] font-mono text-zinc-500">
                  ...
                  <span className="text-amber-400/90 font-bold">SCORE {z.score}</span>
                </div>
              </div>
            ))}
            {(!disp?.zones || disp.zones.length === 0) && (
              <div className="text-zinc-600 text-[9px] italic text-center py-6">
        ...
        {/* Fair value gaps */}
        <div className="apple-glass rounded-lg p-4" id="fair-value-gaps-panel">
          <div className="flex items-center gap-2 text-[9px] font-black tracking-widest text-zinc-400 uppercase mb-3">
            <Layers className="w-3.5 h-3.5 text-sky-400" />
            Fair Value Gaps
            <span className="text-zinc-650 ml-auto font-mono text-[8px]">{(disp?.fvgs || []).length} mapped</span>
          </div>
          <div className="space-y-1.5 max-h-[230px] overflow-y-auto pr-1">
            {(disp?.fvgs || []).slice().reverse().map((f: any) => (
              <div key={f.id} className="bg-black/40 border border-zinc-900 rounded-sm px-2.5 py-1.5 flex items-center justify-between">
                <div>
                  <span className={`text-[9px] font-black uppercase ${f.type === 'bullish' ? 'text-emerald-400' : 'text-rose-400'}`}>
            ...
            {(!disp?.fvgs || disp.fvgs.length === 0) && (
              <div className="text-zinc-600 text-[9px] italic text-center py-6">No open imbalances detected</div>
            )}
          </div>
        </div>

        {/* Liquidity sweeps */}
        <div className="apple-glass rounded-lg p-4" id="liquidity-raids-panel">
          <div className="flex items-center gap-2 text-[9px] font-black tracking-widest text-zinc-400 uppercase mb-3">
          ...
          <div className="space-y-1.5 max-h-[230px] overflow-y-auto pr-1">
            ...
            {(!disp?.sweeps || disp.sweeps.length === 0) && (
              <div className="text-zinc-600 text-[9px] italic text-center py-6">No stops taken in window</div>
            )}
          </div>
        </div>
      </div>
      ...
      {/* ============== CHART WITH ZONES ============== */}
      <div className="apple-glass rounded-lg p-5" id="displacement-overlay-chart-panel">
